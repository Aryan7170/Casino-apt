...
async function main() {
  ...
  try {
    ...
    const Forwarder = await ethers.getContractFactory("GameForwarder");
    ...
    try {
      const Mines = await ethers.getContractFactory("SecureMines");
    ...
    try {
      ...
      const wheel = await Wheel.deploy(tokenAddress, deployerAddress);
    ...
    if (deployedContracts.wheel !== "NOT_DEPLOYED") {
      console.log(
        `npx hardhat verify --network ethereumSepolia ${deployedContracts.wheel} "${deployedContracts.token}" "${deployerAddress}"`
      );
    }
